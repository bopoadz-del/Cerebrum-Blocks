...
const plotIfAvailable = (divId, traces, layout) => {
  if (window.Plotly) {
...
const tableFromObject = (tableEl, obj) => {
  ...
  Object.entries(obj || {}).forEach(([key, value]) => {
    ...
    const shown = Number.isFinite(num) ? num.toFixed(4) : String(value);
    row.innerHTML = `<td>${key}</td><td>${shown}</td>`;
...
const calcSmoothing = async () => {
  ...
  plotIfAvailable(
    "smoothingPlot",
    [{ x: data.clipping_loss_pct, y: data.smoothing_index, type: "scatter", mode: "lines+markers" }],
    { title: "Smoothing index vs clipping loss (%)", xaxis: { title: "Clipping loss (%)" }, yaxis: { title: "Smoothing index" } }
  );
};
...
const bindActions = () => {
  ...
  qs("applyPreset").addEventListener("click", () => {
    ...
    if (preset) {
      ...
      state.histogram = null; // switch back to Weibull
    }
  });
