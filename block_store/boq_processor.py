...
def _resolve_via_project(project_id: str, raw: str) -> str:
    ...
    Returns the input unchanged if no matching document is found or the
    project store isn't reachable.
    """
    ...
    # Pass 2: substring match either direction (truncations / rewordings).
    for doc in docs:
        on = (doc.get("original_name") or "").strip().lower()
...
class BOQProcessorBlock(UniversalBlock):
    ...
    async def process(self, input_data: Any, params: Dict = None) -> Dict:
        ...
        # Callers may pass a bare filename rather than a stored absolute path.
        # Try to resolve that against the project's uploaded documents before
        ...
        try:
            ...
            # set on the server and is a no-op for legacy plaintext files.
