"""Aviation Container - aviation domain container v1.0.

Wraps AviationBlockV2 and routes domain actions. Accepts user custom rules
...
class AviationContainer(UniversalContainer):
    """
    Aviation Container: aviation document analysis, entity extraction,
    metrics, compliance checks, and risk scoring.
    """

    name = "aviation"
    version = "1.0"
    description = "Aviation document analysis, compliance, and risk scoring"
    layer = 3
    tags = ["domain", "container", "aviation"]
    requires = ["aviation_v2"]
    ...
    ui_schema = {
        "input": {
            "type": "text",
            "placeholder": "Paste aviation document text or chain from PDF/OCR...",
            "multiline": True,
        },
        "output": {
            "type": "table",
    ...
    async def route(self, action: str, input_data: Any, params: Dict) -> Dict:
        """Route to internal aviation action handlers."""
    ...
    async def _analyze(self, input_data: Any, params: Dict) -> Dict:
        """Run full AviationBlockV2 analysis."""
        block = self._resolve_block("aviation_v2")
        if block is None:
    ...
    async def _extract_entities(self, input_data: Any, params: Dict) -> Dict:
        """Extract aviation entities only."""
    ...
    async def _calculate_metrics(self, input_data: Any, params: Dict) -> Dict:
        """Calculate aviation metrics only."""
        result = await self._analyze(input_data, params)
        if result.get("status") == "error":
            return result
        return {
