"""Schedule Excel Writer block — thin wrapper over schedule rendering libraries.

The CPM workbook writer lives in ``app.lib.pm_computations.write_schedule_excel``
and the cost-loaded L2 schedule generator lives in ``app.lib.pm_excel``. This
block exposes both under a block-store friendly name.
"""
...
class ScheduleExcelWriterBlock(UniversalBlock):
    ...
    default_config = {
        "output_dir": None,  # None -> system temp directory
    }
    ...
    async def process(self, input_data: Any, params: Dict = None) -> Dict:
        ...
        try:
            if cpm_output:
                from app.lib.pm_computations import write_schedule_excel
                from app.schemas.cpm import CPMOutput, ResourceHistogram

                output = (
                    CPMOutput(**cpm_output)
                    if isinstance(cpm_output, dict)
                    else cpm_output
                )
