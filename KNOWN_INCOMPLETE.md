...
- block_store/kits/construction, block_store/kits/insurance :: **not reproducible from source**. Both were published from The_Fork. 2 of construction's 42 declared artifacts and 12 of insurance's 35 are authored in neither this repo nor the current Fork checkout (`app/containers/construction.py`, `app/data/procedures/procedures_db.json`, `app/data/commission_formulas.json`, `app/knowledge/hkia_gn16_corpus.json`, the four `app/playbooks/*.json`, and others) — `bundle/` holds the only copy. Losing a bundle file loses the content. `publish_kit.py --domain <kit> --refresh` keeps their shared platform code current and reports the orphans; a full republish is impossible until the sources are recovered or re-authored.
