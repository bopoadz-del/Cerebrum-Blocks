...
import pytest
from app.blocks import AuthBlock, MemoryBlock
...
async def test_auth_block():
    ...
    result = await auth.process(
        {"action": "validate", "api_key": "cb_dev_key"},
        {"action": "validate"}
    )
    ...
    result = await auth.process(
        {"action": "validate", "api_key": user_key},
        {"action": "validate"}
    )
    print(f"   Valid: {result['valid']}, Role: {result['role']}")
    ...
    result = await auth.process(
        {"action": "validate", "api_key": "cb_invalid_key"},
        {"action": "validate"}
    )
