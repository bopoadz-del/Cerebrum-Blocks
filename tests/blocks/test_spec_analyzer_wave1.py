...
def _grades(text: str):
    ...
    return {frozenset((g["type"], g["value"])) for g in block._extract_grades(text)}


def test_bs_en_matches_before_bs():
    out = _grades("Concrete per BS EN 1992-1-1 and BS 8500.")
    assert frozenset(("bs_en_standard", "1992-1")) in out
    assert frozenset(("bs_standard", "8500")) in out


def test_astm_trailing_year_and_slash_grade():
    out = _grades("Reinforcement to ASTM A615-22 and ASTM A706/A706M.")
    assert frozenset(("astm_standard", "A615-22")) in out
...
def test_as_and_ibc_codes():
    ...
    assert frozenset(("as_standard", "3600")) in out
    assert frozenset(("ibc_standard", "2021")) in out
