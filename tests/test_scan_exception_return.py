"""B07 — the silent-return twin: an except whose whole body is an empty value.

Ported from The_Fork (READ-ONLY). ``scan_exception_pass`` already fails
the build on ``except Exception: pass``. The larger and quieter version
is the one that returns instead:

    except Exception:
        return None

The caller then cannot tell "there is nothing there" from "the lookup
failed" — same outage, two meanings, one answer.

The twin does not pretend the current sites are fine. It baselines them
by ``file:line`` and fails on the N+1th, so the count can only fall.
...
# Ceiling recorded the day the scanner landed. Raise it only by deleting
# this assertion on purpose. The count is meant to shrink.
RETURN_BASELINE_CEILING = 72
...
def test_the_baseline_is_not_a_place_to_add_things():
    ...
    assert len(RETURN_ALLOWLIST) <= RETURN_BASELINE_CEILING, (
        f"the baseline grew to {len(RETURN_ALLOWLIST)}; it is meant to shrink"
    )


def test_list_returns_prints_paste_ready_keys():
    proc = subprocess.run(
        [sys.executable, "scripts/scan_exception_pass.py", "--list-returns"],
        cwd=REPO,
        capture_output=True,
        text=True,
        check=False,
    ...
    live = all_return_sites()
    if live:
        assert live[0] in proc.stdout
        assert "baseline 2026-09-11" in proc.stdout


def test_ci_runs_the_return_scanner_and_does_not_allow_it_to_fail():
    workflow = CI.read_text(encoding="utf-8")
    assert "python scripts/scan_exception_pass.py" in workflow
    assert "tests/test_scan_exception_return.py" in workflow
    backend = workflow.split("full-suite:", 1)[0]
    idx = backend.find("scan_exception_pass.py")
    assert idx != -1
    window = backend[max(0, idx - 400) : idx]
