"""Price an UNPRICED Bill of Quantities from a typed rate-card.

Given a list of boq_processor line items (description / quantity / unit, with
NO usable rate because the source BOQ is unpriced), it classifies each line,
normalises its unit, and looks up a median rate from the deployed rate-card
(``app/lib/data/boq_rate_card.json``):
...
prototype so this endpoint prices identically to it. Every priced line records
its ``rate_source`` and the sample size ``n`` behind the rate, so confidence
stays visible -- and lines with no comparable rate are kept and flagged
(rate 0, ``rate_source == "NO RATE"``), never dropped and never assigned an
invented number.
"""
...
def categorize(desc: Any) -> str:
    """Classify a line-item description into one of the work categories."""
    d = str(desc).lower()
