...
This module is platform-neutral: callers are responsible for resolving document
ids to document dicts; the extractor only needs ``file_path`` and an optional
``original_name`` key.
"""
...
async def extract_schedule_feed(
    documents: List[Dict[str, Any]],
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Return (procurement_lead_times, target_milestones) distilled from the
    given document dicts via document_engine. Ontology-default lead times are
    dropped (never inject a fabricated default as a real lead time). Milestones
    are deduped. Documents that fail to parse are skipped, not fatal.
    """
    ...
    for doc in documents or []:
        ...
        for m in sched.get("milestones") or []:
            ...
            seen_ms.add(k)
            milestones.append(
                {"name": m.get("name"), "target_date": m.get("target_date")}
            )
    lead_times = [
        {"equipment": k, "lead_time_days": v} for k, v in lead_map.items()
    ]
