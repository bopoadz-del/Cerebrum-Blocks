...
EVM workbook (from PV/EV/AC + BAC):
  ...
  — all LIVE Excel formulas, so the recipient can audit performance.
"""
...
def generate_cost_loaded_schedule(meta: Dict[str, Any], activities: List[Dict[str, Any]]):
    ...
    s = wb.active
    s.title = "L2 Schedule"
    s["A1"] = f"L2 SCHEDULE — {meta.get('project', '')}"
    s["A1"].font = _TITLE
    ...
    for i in order:
        a, c = acts[i], cpm[i]
        s.cell(r, 1, i)
        s.cell(r, 2, a.get("wbs", ""))
        s.cell(r, 3, a.get("name", ""))
        s.cell(r, 4, a.get("duration", 0))
        s.cell(r, 5, ",".join(str(p) for p in a.get("predecessors", [])))
        s.cell(r, 6, c["es"])
        s.cell(r, 7, c["ef"])
        s.cell(r, 8, c["ls"])
        s.cell(r, 9, c["lf"])
        s.cell(r, 10, c["tf"])
        s.cell(r, 11, "YES" if c["critical"] else "")
        cc = s.cell(r, 12, (a.get("cost", 0) if has_cost else None))
        cc.font = _INPUT
        cc.number_format = _MONEY
        r += 1
    s.cell(r, 3, "TOTAL").font = _BOLD
    tc = s.cell(r, 12, f"=SUM(L{first}:L{r - 1})")
    tc.font = _BOLD
    ...
    cl["A1"] = "PROGRESS BASELINE / S-CURVE (cumulative man-days)"
    cl["A1"].font = _TITLE
    period = 7
    ...
    scurve = LineChart()
    scurve.title = "Progress S-Curve (cumulative man-days)"
    scurve.y_axis.title = "Man-days"
    scurve.x_axis.title = "Week"
    sdata = Reference(cl, min_col=4, min_row=3, max_row=slast)
    scats = Reference(cl, min_col=1, min_row=sfirst, max_row=slast)
    scurve.add_data(sdata, titles_from_data=True)
    scurve.set_categories(scats)
    ...
    if has_cost:
        ...
        for idx, i in enumerate(order):
            cl.cell(cr, 1, i)
            cl.cell(cr, 2, acts[i].get("name", ""))
            lk = cl.cell(cr, 3, f"='L2 Schedule'!L{first + idx}")
            lk.font = _XREF
            lk.number_format = _MONEY
    ...
    hist = BarChart()
    hist.type = "col"
    hist.title = "Manpower Histogram (weekly demand)"
    ...
    for label, w, ef, crit in milestones:
        ms.cell(mrow, 1, label)
        ms.cell(mrow, 2, w)
        ms.cell(mrow, 3, ef)
        col = 4
        if start_date:
            ms.cell(mrow, 4, _add_working_days(start_date, ef).isoformat())
            col = 5
    ...
    summ["A1"] = f"SCHEDULE SUMMARY — {meta.get('project', '')}"
    summ["A1"].font = _TITLE
    summ["B3"] = "Project Duration (days)"
    summ["C3"] = f"='L2 Schedule'!D{total_cost_row + 2}"
    summ["B4"] = "Total Man-days"
    summ["C4"] = f"='Manpower Histogram'!E{rr}"
    summ["B5"] = "Peak Manpower"
    summ["C5"] = f"='Manpower Histogram'!C{plast + 2}"
    summ["B6"] = "Critical Activities"
    summ["C6"] = sum(1 for i in order if cpm[i]["critical"])
    summ["B7"] = "Milestones"
    summ["C7"] = len(milestones)
    if has_cost:
        summ["B8"] = f"Total {cost_label} ({ccy})"
        tcl = summ["C8"]
        tcl.value = f"='L2 Schedule'!L{total_cost_row}"
        tcl.font = _XREF
        tcl.number_format = _MONEY
...
def generate_evm_workbook(meta: Dict[str, Any], periods: List[Dict[str, Any]]):
    ...
    e = wb.active
    e.title = "EVM"
    e["A1"] = f"EARNED VALUE MANAGEMENT — {meta.get('project', '')}"
    e["A1"].font = _TITLE
