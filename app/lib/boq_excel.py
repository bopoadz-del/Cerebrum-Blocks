...
recipient can audit Rate x Qty = Amount, exactly like a real priced BOQ.
...
def generate_cost_boq(
    ...
    for cat in categories:
        ...
        for it in cat.get("items", []):
            ...
            qc = det.cell(row=r, column=5, value=it.get("qty", 0))
            qc.font = _INPUT
            rc = det.cell(row=r, column=6, value=it.get("rate", 0))
            rc.font = _INPUT
            rc.number_format = _MONEY
            ac = det.cell(row=r, column=7, value=f"=E{r}*F{r}")
            ac.number_format = _MONEY  # LIVE
            r += 1
        end = r - 1
    ...
    for j, h in enumerate(
        ...
        c = summ.cell(row=sh, column=j, value=h)
    ...
    cov["B2"] = meta.get("title", "Bill of Quantities")
    cov["B2"].font = Font(bold=True, size=18)
    cov["B4"] = "Project:"
    cov["C4"] = meta.get("project", "")
    cov["B5"] = "Location:"
    cov["C5"] = meta.get("location", "")
    cov["B6"] = "Currency:"
    cov["C6"] = ccy
    cov["B7"] = "Date:"
    cov["C7"] = meta.get("date", "")
    cov["B9"] = "Contractor Cost Summary"
    cov["B9"].font = _BOLD
    cov["B10"] = "Construction Cost"
    cc = cov["C10"]
    cc.value = f"=IFERROR(BOQ_Summary!D{total_row},0)"
    cc.font = _XREF
    cc.number_format = _MONEY
    cov["B11"] = "TOTAL PROJECT COST"
    tp = cov["C11"]
    tp.value = f"=IFERROR(BOQ_Summary!D{total_row},0)"
    tp.font = Font(bold=True, color="008000")
    tp.number_format = _MONEY
    cov.sheet_view.showGridLines = False

    # ── Cost_Charts ─────────────────────────────────────────────────────────
    ch = wb.create_sheet("Cost_Charts")
    ch["B2"] = "COST VISUALIZATION"
    ch["B2"].font = _TITLE
    ch.cell(row=5, column=2, value="Category")
    ch.cell(row=5, column=3, value=f"Amount ({ccy})")
    cr = 6
    ...
    ch.add_chart(pie, "E5")
    bar = BarChart()
    bar.title = "Category Cost Comparison"
...
def evaluate_workbook_total(src) -> float:
    ...
    def _eval(sheet: str, expr: str) -> float:
        ...
        if m:
            return cell(sheet, m.group(1).replace("$", "")) * cell(
                sheet, m.group(2).replace("$", "")
            )
        m = re.fullmatch(r"([A-Z]+\$?\d+)\+([A-Z]+\$?\d+)", expr)
        if m:
            return cell(sheet, m.group(1).replace("$", "")) + cell(
