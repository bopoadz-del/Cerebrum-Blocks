...
class ClashTriageBlock(UniversalBlock):
    """Ranks clash findings into a build order: resolution rank, zone congestion, workflow-noise filtering, programme CSV loading."""

    name = "clash_triage"
    version = "1.0.0"
    description = "Ranks clash findings into a build order: resolution rank, zone congestion, workflow-noise filtering, programme CSV loading."
    layer = 3
    tags = ['mep', 'construction', 'coordination', 'workflow']
    requires = []
    ...
    _ACTIONS = {'triage': ('triage', ['findings', 'elements_by_id', 'programme', 'order', 'cell_m']), 'rank': ('resolution_rank', ['system', 'order']), 'congestion': ('zone_congestion', ['elements', 'cell_m']), 'order': ('load_order', ['path']), 'programme': ('load_programme', ['csv_path']), 'clash_id': ('clash_id_for', ['a', 'b'])}

    async def process(self, input_data, params=None):
        """Dispatch to the module function for the requested action."""
        params = params or {}
        payload = input_data if isinstance(input_data, dict) else {}
        action = str(payload.get("action", "triage")).lower()
