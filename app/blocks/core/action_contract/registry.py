...
from app.blocks.core.action_contract.models import ActionContext, ActionSpec

logger = logging.getLogger(__name__)

# Domain-kit action modules are discovered under this package as
# ``app.blocks.domains.<kit>.actions`` exposing an ``ACTION_SPECS`` list.
# Override at runtime by passing ``package=`` to ``discover()``.
