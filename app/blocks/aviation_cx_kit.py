"""Aviation CX Kit — read-only customer-experience intelligence.

Scope (enforced in code):
- READS:  customer-experience docs in the project corpus (via RAG), query
- WRITES: grounded + cited CX output
- NEVER:  fabricate domain figures (gate-enforced); never write to external
          ops systems; never cross project tenancy.
...
class AviationCxKitBlock(TypedBlock):
    ...
    version = "1.0.0"
    description = (
        "Read-only customer-experience intelligence: policies, feedback, "
        "and service standards grounded in project documents."
    )
    layer = 3
    tags = ["aviation", "cx", "customer_experience", "domain", "intelligence"]
    ...
    ui_schema = {
        "input": {
            "type": "json",
            "placeholder": '{"query": "What is the compensation policy?", "project_id": "..."}',
            "multiline": True,
        },
        "output": {
            "type": "json",
        ...
        "quick_actions": [
            {"icon": "🤝", "label": "CX lookup", "prompt": "What do the customer-experience documents say?"},
        ],
    }
    ...
    async def process(self, input_data: Any, params: Dict = None) -> Dict:
        ...
        try:
            ...
            logger.exception("CX retrieval failed for project %s", project_id)
            return self._error(f"Retrieval failed: {exc}")

        try:
            draft = await self._generate_answer(query, citations)
        except Exception as exc:  # noqa: BLE001
            logger.exception("CX generation failed for project %s", project_id)
            return self._error(f"Generation failed: {exc}")

        try:
            grounding = await self._ground(query, draft, citations)
        except Exception as exc:  # noqa: BLE001
            logger.exception("CX grounding gate failed for project %s", project_id)
        ...
        if verdict == "block":
            return {
                "status": "success",
                "domain": self.config.get("domain", "cx"),
                "verdict": "block",
                "answer": None,
                "citations": citations,
                "grounding": grounding,
    ...
    async def _generate_answer(self, query: str, citations: List[Dict]) -> str:
        ...
        system_prompt = (
            "You are an aviation customer-experience assistant. "
            "Answer ONLY from the retrieved context. "
            "Do not invent policies, compensation amounts, or service standards. "
            "Cite sources using [n] notation. "
            "This is read-only intelligence — never modify passenger records."
        )
        prompt = f"Context:\n{context_text}\n\nQuestion: {query}"
