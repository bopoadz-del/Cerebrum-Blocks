...
class BcfExportBlock(UniversalBlock):
    """Exports findings to a BCF 2.1 zip (markup XML + viewpoints when provided) and validates a produced zip."""

    name = "bcf_export"
    version = "1.0.0"
    description = "Exports findings to a BCF 2.1 zip (markup XML + viewpoints when provided) and validates a produced zip."
    layer = 3
    tags = ['mep', 'construction', 'coordination', 'bcf']
    requires = []
    ...
    _ACTIONS = {'export': ('export_bcf', ['findings', 'out_path', 'model_name', 'viewpoints', 'rule_lookup']), 'validate': ('validate_bcf_zip', ['path'])}
