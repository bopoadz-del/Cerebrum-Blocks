...
class ClearanceRulesBlock(UniversalBlock):
    """Clearance rule table loader and evaluator: category pairs + axes with precedent resolution; rules must carry a citation."""

    name = "clearance_rules"
    version = "1.0.0"
    description = "Clearance rule table loader and evaluator: category pairs + axes with precedent resolution; rules must carry a citation."
    layer = 3
    tags = ['mep', 'construction', 'coordination', 'rules']
    requires = []
    ...
    _ACTIONS = {'evaluate': ('evaluate', ['findings', 'rules']), 'find': ('find_applicable_rule', ['rules', 'category_a', 'category_b', 'axis']), 'load': ('load_rules', ['path_or_list']), 'resolve': ('resolve_precedence', ['candidates'])}
