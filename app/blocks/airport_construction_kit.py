"""airport and aviation construction reasoning kit — store-facing block.

Identity only. Every rule this block enforces is DECLARED in
``app/blocks/airport_construction/manifest.yaml`` (vocabulary) and ``invariants.yaml``
...
class AirportConstructionKitBlock(ReasoningKitBlock):
    name = "airport_construction_reasoning"
    kit_name = "airport_construction"
    description = (
        "Declarative airport and aviation construction reasoning kit on the shared reasoning layer: eight "
    ...
    tags = ['airport', 'aviation', 'construction']
