"""offshore marine operations reasoning kit — store-facing block.

Identity only. Every rule this block enforces is DECLARED in
``app/blocks/offshore_marine/manifest.yaml`` (vocabulary) and ``invariants.yaml``
...
class OffshoreMarineKitBlock(ReasoningKitBlock):
    ...
    tags = ['offshore', 'marine', 'installation']
