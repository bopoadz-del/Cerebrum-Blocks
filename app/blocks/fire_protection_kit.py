"""fire protection and firefighting systems reasoning kit — store-facing block.

Identity only. Every rule this block enforces is DECLARED in
``app/blocks/fire_protection/manifest.yaml`` (vocabulary) and ``invariants.yaml``
...
class FireProtectionKitBlock(ReasoningKitBlock):
    ...
    tags = ['fire', 'sprinkler', 'protection']
