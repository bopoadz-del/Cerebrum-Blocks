"""water treatment plant operations reasoning kit — store-facing block.

Identity only. Every rule this block enforces is DECLARED in
``app/blocks/water_treatment/manifest.yaml`` (vocabulary) and ``invariants.yaml``
...
class WaterTreatmentKitBlock(ReasoningKitBlock):
    ...
    tags = ['water', 'treatment', 'disinfection']
