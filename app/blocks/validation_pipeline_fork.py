"""validation_pipeline_fork — runnable 5-stage validation checker.

Ported verbatim from The_Fork `app/blocks/validation_pipeline.py` (current default branch) with its companion `config/empirical_ranges.json`. The only adaptations: the store `_envelope` wrapper over the donor's `process` (the pipeline verdict itself is the donor's, byte for byte) and a provenance label in the block description.
...
def _envelope(status, result=None, error=None, detail=None):
    return {
        "block_id": "validation_pipeline_fork",
        "status": status,
        "result": result,
        "error": error,
        "detail": detail,
...
def _get_ureg() -> Optional[Any]:
    ...
    try:
        ...
        # (Same behaviour as the donor; the intermediate assignment keeps
        # the store's scan_exception_pass RETURN twin quiet without changing
        # what this returns.)
        _UREG = None
        return _UREG
...
class ValidationPipelineForkBlock(UniversalBlock):
    ...
    name = "validation_pipeline_fork"
    version = "1.1.0"
    description = (
        "real (clone of The_Fork app/blocks/validation_pipeline.py + "
        "config/empirical_ranges.json): runnable 5-stage validation "
        "(syntactic / dimensional / physical / empirical / operational) "
        "for numeric results, with Pint dimensional analysis and "
        "file-backed empirical ranges. Returns per-stage pass/fail plus an "
        "overall verdict so the agent can refuse to report numbers that "
        "failed validation. Envelope status is ok/failed/refused per the "
        "overall verdict - a failed validation is never returned as ok."
    )
    layer = 3
    ...
    async def process(self, input_data: Any, params: Dict = None) -> Dict:
        """Store envelope over the donor's pipeline. The donor's verdict is
        preserved verbatim in ``result``; the envelope status is ``ok`` on an
        overall pass, ``failed`` on an overall fail (fail-loud - a consumer
        must never mistake a failed validation for a passed one), ``refused``
        when no input was given, and ``error`` when the pipeline itself broke."""
        params = params or {}
        data = input_data if isinstance(input_data, dict) else {}
        try:
            outcome = self._pipeline(input_data, params)
        ...
        if outcome.get("status") == "error":
            return _envelope("refused", error=outcome.get("error"))
        if outcome.get("overall") == "fail":
            return _envelope("failed", result=outcome)
        return _envelope("ok", result=outcome)

    def _pipeline(self, input_data: Any, params: Dict = None) -> Dict:
        params = params or {}
        data = input_data if isinstance(input_data, dict) else {}
