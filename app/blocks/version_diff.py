...
class VersionDiffBlock(UniversalBlock):
    """Diffs two finding sets (resolved / new / regressed / persisting) and scores proposals against the diff."""

    name = "version_diff"
    version = "1.0.0"
    description = "Diffs two finding sets (resolved / new / regressed / persisting) and scores proposals against the diff."
    layer = 3
    tags = ['mep', 'construction', 'coordination', 'versioning']
    requires = []
    ...
    _ACTIONS = {'diff': ('diff_versions', ['findings_v1', 'findings_v2']), 'score': ('score_proposals', ['proposals', 'diff'])}

    async def process(self, input_data, params=None):
        """Dispatch to the module function for the requested action."""
        params = params or {}
        payload = input_data if isinstance(input_data, dict) else {}
        action = str(payload.get("action", "diff")).lower()
