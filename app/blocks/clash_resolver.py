...
class ClashResolverBlock(UniversalBlock):
    """Proposes collision-free moves for clash items, preserving fall direction; batch verification against zone elements."""

    name = "clash_resolver"
    version = "1.0.0"
    description = "Proposes collision-free moves for clash items, preserving fall direction; batch verification against zone elements."
    layer = 3
    tags = ['mep', 'construction', 'coordination', 'resolution']
    requires = []
    ...
    _ACTIONS = {'resolve': ('resolve', ['item', 'element', 'neighbours', 'required_gap_mm', 'rule_ids', 'clause_text', 'margin_mm']), 'candidates': ('candidate_moves', ['item', 'element', 'required_gap_mm', 'margin_mm', 'steps']), 'verify': ('verify_batch', ['proposals', 'elements_by_id', 'zone_elements', 'required_gap_mm', 'zone_key'])}

    async def process(self, input_data, params=None):
        """Dispatch to the module function for the requested action."""
        params = params or {}
        payload = input_data if isinstance(input_data, dict) else {}
        action = str(payload.get("action", "resolve")).lower()
