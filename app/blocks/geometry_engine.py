...
class GeometryEngineBlock(UniversalBlock):
    """Exact clash and clearance measurement from one pass over IFC meshes: AABB pre-filter, then triangle-mesh judgement (CLASH vs CLEARANCE)."""

    name = "geometry_engine"
    version = "1.0.0"
    description = "Exact clash and clearance measurement from one pass over IFC meshes: AABB pre-filter, then triangle-mesh judgement (CLASH vs CLEARANCE)."
    layer = 3
    tags = ['mep', 'construction', 'coordination', 'geometry']
    requires = []
    ...
    _ACTIONS = {'judge': ('judge_pair', ['element_a', 'element_b', 'mesh_a', 'mesh_b', 'required_clearance_m', 'rule_id', 'category_a', 'category_b']), 'aabb_overlaps': ('aabb_overlaps', ['box_a', 'box_b', 'pad']), 'backend': ('exact_backend_available', [])}
