"""aviation operations reasoning kit — store-facing block.

Identity only. Every rule this block enforces is DECLARED in
``app/blocks/aviation_ops/manifest.yaml`` (vocabulary) and ``invariants.yaml``
...
class AviationOpsKitBlock(ReasoningKitBlock):
    ...
    tags = ['aviation', 'operations', 'dispatch']
