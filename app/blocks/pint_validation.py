...
def _get_ureg() -> Optional[Any]:
    ...
    try:
        ...
        logger.debug("Pint UnitRegistry unavailable (%s); string checks only", exc)
        return None
...
def _envelope(status, result=None, error=None, detail=None):
    return {"block_id": "pint_validation", "status": status, "result": result, "error": error, "detail": detail}


class PintValidationBlock(UniversalBlock):
    ...
    async def execute(self, input_data: Any, params: Dict = None) -> Dict:
        """Envelope wrapper over the donor's process() payload."""
        payload = await self.process(input_data, params)
        if payload.get("status") in ("error", "refused"):
            return _envelope("error", error=payload.get("error"), detail=payload)
        return _envelope("ok", result=payload)
